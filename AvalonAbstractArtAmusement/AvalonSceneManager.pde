...
class SceneManager {
     ...
     private int SizeX, SizeY;
      ...
      public List<AbstractButton> ButtonList= new ArrayList<AbstractButton>();
      public boolean BOOL_AbstractButtons= false;
      private BrokenButton StartButton;
      ...
      public int makeButton()
      {
        ...
        StartButton= new BrokenButton(
                              idx,
                              "Start", 
                              this.GUI_FILEPREFIX + StartButtonTexture, 
                              "Start", 
                              StartPositionX, 
                              StartPositionY);
        StartButton.setPosX(StartPositionX - StartButton.BrokenWidth / 2);
        StartButton.setPosY(StartPositionY - StartButton.BrokenHeight / 2);
           this.StartButton.setActiveAction(new EventHandler(){
            @Override
            public void callback(int UID)
            {
              setSelectedHexButton(UID);
...
*/ 
     ...
     public void drawDottedLine(
      ...
      public void update()
      {
        ...
        if(BOOL_AbstractButtons == true)
          for(int aa= 0; aa < ButtonList.size(); aa++){
            ButtonList.get(aa).update();
          } 
      ...
      {
        for(int i= 0; i < AbstractArtworksSum; i++){
          AbstractArtworks[i]= loadImage(AbstractArtworkName[i]);
          AbstractArtworks[i].loadPixels();
        }
        SizeX= SceneSizeX;
        SizeY= SceneSizeY;
      ...
      public float[] getStartButtonPos()
      {
        float Pos[]={StartButton.getPosX(), StartButton.getPosY()};
        return Pos;
      }
};
