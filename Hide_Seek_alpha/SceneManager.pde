...
class SceneManager {
     ...
     private int SizeX, SizeY;
      ...
      private float PreMouseClickX, PreMouseClickY, MouseClickX, MouseClickY;
      ...
      float PanLeftTrigger= width / 10;
      float PanRightTrigger= width - PanLeftTrigger;
      float PanTopTrigger= height / 8;
      float PanDownTrigger= height - PanTopTrigger;
      float PanLeftMax= -750;
      float PanRightMax= 150;
      float PanTopMax= -800;
      float PanDownMax= 200;
      float PanXRate= 0.5;
      float PanYRate= 0.5;
      float PanXUnit= 2;
      float PanYUnit= 2;
      ...
      float getPanXSpeed(float Ratio)
      {
        return (exp(Ratio) * 0.1);
      }
      
      float getPanYSpeed(float Ratio)
      {
        println(Ratio);
        return (exp(Ratio) * 0.1);
      } 
      ...
      {
        return mouseX - ScreenStartX;
      }
      
      public float getMouseY()
      {
        return mouseY - ScreenStartY;
      }
        ...
        for(idx= 0; idx < HexButtonSum; idx++){
          ...
          this.HexButtons[idx].setActiveAction(new EventHandler(){
            ...
            public void callback(int UID)
            {
              ...
              MouseClickX= getMouseX();
              MouseClickY= getMouseX();
              PaintMode= UID;
          ...
          ReplayButton= new Button(
                                idx,
                                this.ButtonName[idx], 
                                "data/GUI/ButtonIcon/replay.png", 
                                ButtonHelp[idx], 
                                width / 16, 
                                90);
          this.ReplayButton.setActiveAction(new EventHandler(){
            @Override
            public void callback(int UID)
            {
              SceneMgr.init();
          ...
          HelpButton= new Button(
                                idx,
                                this.ButtonName[idx], 
                                "data/GUI/ButtonIcon/help.png", 
                                ButtonHelp[idx], 
                                width / 16 * 2, 
                                90);
          this.HelpButton.setActiveAction(new EventHandler(){
            @Override
            public void callback(int UID)
            {
              //SceneMgr.init();
...
*/ 
     ...
     public void drawDottedLine(
      ...
      void updateScreenPan()
      {
        if(mouseX < PanLeftTrigger && ScreenStartX <= PanRightMax){
          this.ScreenStartX -= (mouseX - PanLeftTrigger) * getPanXSpeed((mouseX - PanLeftTrigger) / -PanLeftTrigger) * 0.5;
        }
        if(mouseX > PanRightTrigger && ScreenStartX >= PanLeftMax){
          this.ScreenStartX += (PanRightTrigger - mouseX) * getPanXSpeed((PanRightTrigger - mouseX) / PanRightTrigger) * 1.5;
        }
        if(mouseY < PanTopTrigger && ScreenStartY <= PanDownMax){
          this.ScreenStartY -= (mouseY - PanTopTrigger) * getPanYSpeed((mouseY - PanTopTrigger) / -PanTopTrigger);
        }
        if(mouseY > PanDownTrigger && ScreenStartY >= PanTopMax){
          this.ScreenStartY += (PanDownTrigger - mouseY) * getPanYSpeed((PanDownTrigger - mouseY) / PanDownTrigger) * 3;
        }
      }
      ...
      public void makeCharacters()
      {
       ...
       PurRock01= new Rock("PurPawn01", 2);
       PurRock02= new Rock("PurPawn02", 60);
       PurRock03= new Rock("PurPawn03", 35);
       PurRock04= new Rock("PurPawn03", 55);
      }
