  ...
  void loadAnimation(String AnimFilePrefix, PImage ImageArray[])
  {
    ...
    void findPathByMousePosition()
    {
        if(PointedCubeIndex != -1){
          PathResult= findPath(this.getStandingCubeIndex(), PointedCubeIndex);
          if(PathResult != null){
    ...
    {
      PathResult= findPath(this.getStandingCubeIndex(), TargetIndex);
      if(PathResult != null){
        PathArray= PathResult.toArray();
        PathArrayIndex= 0;
        if(DEBUG_MODE){
   ...
   int getIdleAnimIndex()
 {
  ...
  void drawSelf()
  {
    if(AnimCounter++ >= Interval){
      ...
      image(AnimArray[RenderingAnimIndex][AnimIndex % (AnimArray[RenderingAnimIndex].length)], PosX + DisplacementX, PosY + DisplacementY, 120, 120);
...
class Goos extends Characters{
  ...
  void drawSelf()
  {
    if(AnimCounter++ >= Interval){
      ...
      //if(PosX != TargetX) PosX+= DX;
      //if(PosY != TargetY) PosY+= DY;
      image(GooAnim.AnimArray[RenderingAnimIndex][AnimIndex % (GooAnim.AnimArray[RenderingAnimIndex].length)], PosX + DisplacementX, PosY + DisplacementY, 90, 180);
      //if((AnimIndex % (AnimArray[RenderingAnimIndex].length) == 0) && (RenderingAnimIndex == WAVINGHAND))AnimTimes+= 1;
  }
